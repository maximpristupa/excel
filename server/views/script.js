...
function displayPage(response = null) {
  ...
  csv.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, cellIndex) => {
      html += `<div class="cell"> <input id="${rowIndex}_${cellIndex}" onmouseover="backlight('${rowIndex}_${cellIndex}', '#cacaca')" onmouseout="backlight('${rowIndex}_${cellIndex}', '#f3f3f5')" class="cellInput cell" onclick="openInput(this)" onkeydown = "if (event.keyCode == 13) calculate()" type="text" name="cell" value="${cell}"> </div>`;
    });
    html += '</div>';
  });
...
function printFiles(json) {
  ...
  html = '';
  if (json === '{}') {
    html += '<p class="chooseFile">No files to open</p>';  
    modal.innerHTML = html;
    return;
  }
  html += '<p class="chooseFile">Choose File</p>';
